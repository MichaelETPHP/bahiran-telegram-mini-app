...
- **Global scope**: `C:/SVELT_APP/underMentenance/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/SVELT_APP/underMentenance/.claude/notes/`)
...
2. Glob `C:/SVELT_APP/underMentenance/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/SVELT_APP/underMentenance/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/SVELT_APP/underMentenance/.claude/notes/` — works in any directory
