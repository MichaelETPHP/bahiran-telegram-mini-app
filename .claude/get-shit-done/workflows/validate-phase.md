...
```bash
AUDITOR_MODEL=$(node "C:/SVELT_APP/underMentenance/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "C:/SVELT_APP/underMentenance/.claude/get-shit-done/bin/gsd-tools.cjs" config get workflow.nyquist_validation --raw)
```
...
```
Task(
  prompt="Read C:/SVELT_APP/underMentenance/.claude/agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `C:/SVELT_APP/underMentenance/.claude/get-shit-done/templates/VALIDATION.md`
...
node "C:/SVELT_APP/underMentenance/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
