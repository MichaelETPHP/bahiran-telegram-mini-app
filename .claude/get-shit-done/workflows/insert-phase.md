...
```bash
INIT=$(node "C:/SVELT_APP/underMentenance/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "C:/SVELT_APP/underMentenance/.claude/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
